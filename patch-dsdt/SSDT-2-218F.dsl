 ...
 *
 * Disassembly of SSDT-2-218F.aml, Sun Apr  7 12:36:26 2019
 *
...
DefinitionBlock ("", "SSDT", 1, "HPQOEM", "218F    ", 0x00003000)
{
    ...
    Scope (\_PR)
    {
        OperationRegion (PPMT, SystemMemory, 0xAAFB9F18, 0x0029)
    ...
    Scope (\_PR.CPU0)
    {
        ...
        Method (CTLD, 0, NotSerialized)
        {
            If (LEqual (\ECON, One))
            {
                If (LAnd (CondRefOf (\_PR.CNTD), CondRefOf (\_SB.PTID.WPMD)))
                {
                    If (\_PR.CNTD)
                    {
                        Name (BUFF, Buffer (0x1A)
                        {
                            /* 0000 */  0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0x30, 0x0A,
